...
# -------------------------------------------------------------------------
# REPLACE 'NYU_EMAIL' with your NYU email address registered in Gradescope
# -------------------------------------------------------------------------
salt = b'Tandon'                  # byte-object as required
password = 'NYU_EMAIL'            # <-- replace this with your NYU email
input_string = 'AlwaysWatching'   # secret data
...
dns_records = {
    ...
    'nyu.edu.': {
        ...
        dns.rdatatype.TXT:  (str(encrypted_value),),         # str() cast of encrypted bytes
        dns.rdatatype.MX:   [(10, 'mxa-00256a01.gslb.pphosted.com.')],
        dns.rdatatype.AAAA: '2001:db8:85a3::8a2e:373:7312', # RFC 5952 compressed
...
def run_dns_server():
    # UDP socket on loopback, standard DNS port 53
    ...
    while True:
        try:
            ...
            if qname in dns_records and qtype in dns_records[qname]:
                ...
                if qtype == dns.rdatatype.MX:
                    for pref, server in answer_data:
                        rdata_list.append(MX(dns.rdataclass.IN, dns.rdatatype.MX,
                                             pref, dns.name.from_text(server)))
                elif qtype == dns.rdatatype.SOA:
                    mname, rname, serial, refresh, retry, expire, minimum = answer_data
                    rdata = SOA(dns.rdataclass.IN, dns.rdatatype.SOA,
                                dns.name.from_text(mname), dns.name.from_text(rname),
                ...
                # Return NXDOMAIN for unknown domains or unsupported record types
                response.set_rcode(dns.rcode.NXDOMAIN)

            # Set the AA (Authoritative Answer) flag using bitwise operations
            response.flags |= 1 << 10
            ...
            continue  # keep server alive on bad packets
